...
namespace QBS.ServiceLocator
{
	...
	public static class ServiceLocator
	{
		...
		public static void GameStart()
		{
			...
			SetupContextServicesMap();
			GetAllServiceAttributedTypes();
			DiscoverServicesOfLifetime(Lifetime.Global);
		}

		private static void SetupContextServicesMap()
		{
			_contextServiceContainers = new Dictionary<Context, ServiceContainer>();
			for (var i = 1; i < (int)Context._Count; i++)
			{
				_contextServiceContainers.Add((Context)i, null);
			}
		}
		...
		public static void PurgeContainer(Lifetime lifetime, Context context = Context.None)
		{
			switch (lifetime)
			{
				...
				case Lifetime.ScopedContext:
				{
					if (_contextServiceContainers == null)
					{
						Debug.LogError("Context Service Map not initialized");
						return;
					}
					...
					if (_contextServiceContainers.TryGetValue(context, out var container))
					{
						...
						container.DisposeContainer();
						_contextServiceContainers[context] = null;
					}
		...
		public static void DiscoverServicesOfLifetime(Lifetime lifetime, Context context = Context.None)
		{
			switch (lifetime)
			{
				case Lifetime.ScopedContext:
				{
					...
					{
						Debug.LogError("Context cannot be None for scoped context");
						return;
					}

					if (_contextServiceContainers.TryGetValue(context, out var value))
					{
						if (value != null)
						{
						...
						var newContextContainer =
							new ServiceContainer(Lifetime.ScopedContext, _allServicesMap, context);
				...
				case Lifetime.Global:
				{
					...
					{
						Debug.LogError("Global Services already Discovered");
						return;
					}
		...
		public static void RegisterSceneService<TService>(TService service) where TService : class, IService
		{
			...
			{
				Debug.LogError("Scene Service Container for scene was not initialized");
				return;
			}
